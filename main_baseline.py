...
def train(cur_iter, iter_per_epoch, epoch, data_loader, model, criterion, optimizer, scheduler, opt):
    ...
    c = Configure(sample_duration=opt.sample_duration, data_type=opt.train_data_type)
    ...
    while i < total_iter:
        ...
        for _, (inputs, targets) in enumerate(data_loader):
            ...
            # logging iteration #, loss, lr, batch_time
            # and draw log to tensorboard
            # per 10 iterations
            if i % 10 == 0:
            ...
            # print epoch_accuracy per an epoch
...
def train_dataset(opt, model):
    ...
    # 19.8.7. add
    # Multiloss add
    assert opt.loss_type in ['KDloss', 'normal']
    if opt.loss_type == 'KDloss':
        criterion = KDloss(loss_type=opt.KDloss_type)
    else:
