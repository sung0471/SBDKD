...
def set_dataset_cfg():
    ...
    dataset_cfg = {
        "ClipShots": {
            ...
            "train_list_path": 'data/data_list/deepSBD_new.txt',
            # "train_list_path": 'data/data_list/detector.txt',
            "gt_dir": 'annotations/test_origin.json'
...
def set_model_cfg():
    model = opt.model
    assert model in ['alexnet', 'resnet', 'resnext']
...
def set_cfg(parser_class: argparse.ArgumentParser):
    ...
    # `20.1.7. move from main_baseline.py/main()
    # parser.add_argument('--n_classes', default=3, type=int, help='Number of classes, automatic set by train_data_type'
